<template>
  ...
  <section class="news">
    ...
    <div class="px-4 mb-5 md:px-20 flex flex-col md:flex-row gap-10">
      <div class="flex-1">
        <router-link to="/berita/tim-poliwangi-raih-award-of-excellence-2nd-winner-pada-ajang-internasional" class="bg-white overflow-hidden cursor-pointer">
          <img src="/src/assets/img/berita_1.png" alt="Berita Utama" class="w-full h-75 object-cover" />
          <div class="py-3">
            <h3 class="text-xl font-bold text-[var(--text-black)] mb-2 text-justify hover:text-yellow-400">Tim Poliwangi Raih Award of Excellence (2nd Winner) pada ajang internasional.</h3>
            <div class="flex items-center text-sm text-[var(--text-black)]">
              <p><i class="fa-solid fa-calendar-days me-2"></i>21 Maret 2002</p>
              <span class="mx-2">|</span>
              <p><i class="fa-solid fa-tags me-2"></i>Berita</p>
            </div>
            <p class="text-[var(--text-black)] mb-4 text-justify">Februari 2025, Poliwangi mengawali awal tahun dengan prestasi internasional yaitu sebagai “The 2nd Winner dengan...</p>
          </div>
        </router-link>
      </div>
  ...
  <section class="service relative w-full h-[400px] md:h-[500px] my-20">
    ...
    <div class="absolute inset-0">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 px-4 md:px-20">
        <a v-for="(item, index) in serviceList" :key="index" :href="item.url" target="_blank" rel="noopener noreferrer" class="bg-white overflow-hidden rounded-lg cursor-pointer hover:shadow transition block">
